...
interface Props {
  ...
  attendanceBySubject: AttBySubjectItem[];
  engagementSegments: EngagementItem[];
  topStudents: TopStudentItem[];
  gradeTrend: GradeTrendItem[];
}
...
export default function TeacherDashboardClient({
  ...
  return (
    <div>
      ...
      {/* Charts row 2 */}
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        {/* Avg Grade Trend over time */}
        <Col xs={24} lg={12}>
          <Card title="Average Grade Trend">
            {gradeTrend.length === 0 ? (
              <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No grade data" />
            ) : (
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={gradeTrend} margin={{ top: 10, right: 16, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" />
