...
import { Typography, Table, Avatar, Input, Tag, Button } from "antd";
import { UserOutlined, SearchOutlined, EyeOutlined } from "@ant-design/icons";
import { useRouter, usePathname } from "next/navigation";
import type { ColumnsType } from "antd/es/table";
import DeanStudentDetailDrawer from "./DeanStudentDetailDrawer";
...
export default function DeanStudentsClient({
  students,
  total,
  page,
  pageSize,
  ...
  const navigate = (updates: Record<string, string | number | undefined>) => {
    ...
    const merged = {
      ...
      search,
      ...Object.fromEntries(
        Object.entries(updates).map(([k, v]) => [k, v != null ? String(v) : ""])
      ),
    };
  ...
  const columns: ColumnsType<Student> = [
    {
      ...
      render: (_, record) => (
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <Avatar icon={<UserOutlined />} src={record.photo_signed_url ?? undefined} size={36} />
          <div>
    ...
    },
    {
      ...
      render: (_, record) => (
        <Button
          ...
          onClick={(e) => {
            e.stopPropagation();
            openDrawer(record.id, record.full_name);
          }}
        >
          Student Profile
        </Button>
